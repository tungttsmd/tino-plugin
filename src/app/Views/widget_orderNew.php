<form method="POST" style="max-width: 100%; margin: 20px 0;">
    <div style="margin-top: 10px; display: flex; gap: 20px">
        <input type="text" id="nameserverInput" name="domain" placeholder="nameserver: tenmien.vn" style="padding-left: 10px; padding-right: 10px; width: 100%; flex: 7" value="<?= $widget->data->domain ?? '' ?>">
        <input type="text" id="domainInput" name="domain" placeholder="nhập tên miền bạn muốn mua: tenmien.vn" style="padding-left: 10px; padding-right: 10px; width: 100%; flex: 7" value="<?= $widget->data->domain ?? '' ?>">
        <button type="button" id="domainInspect" name="button" style="flex: 3">Kiểm tra</button>
        <button type="button" id="domainToOrder" name="button" style="flex: 3">Đặt hàng/Xem đơn hàng</button>
    </div>
</form>
